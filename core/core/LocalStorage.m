...
#define TABLENAME @"localStroage.sqlite"
...
//    static dispatch_
    ...
    static dispatch_once_t database_once_oken;
    dispatch_once(&database_once_oken, ^{
...
+(int)getItemID:(NSString*)key{
    ...
    if (sqlite3_prepare_v2([LocalStorage database], [quary UTF8String], -1, &stmt, &errorMsg) == SQLITE_OK) {
...
//+(void)setItem:(NSString*)key value:(NSString*)value{
    ...
    Json * json = [[Json alloc] initWithObject:value];
    
    NSString * valueString = [json description];
    
    char *update;// = "INSERT OR REPLACE INTO DATA_STRORAGE(KEY,VALUE)VALUES(?,?);";
    
    int id = [self getItemID:name];
    if (id == -1){
    ...
    sqlite3_stmt *stmt;
    //
    if (sqlite3_prepare_v2([LocalStorage database], update, -1, &stmt, NULL) == SQLITE_OK) {
        //
        ...
        sqlite3_bind_text(stmt, 2, [valueString UTF8String], -1, NULL);
        if (id != -1){
...
+(NSObject *)getItem:(NSString *)name{
    return [LocalStorage getItemImpl:name];
}

+(NSObject *)getItem:(NSString *)name type:(Class)type{
    return [[type alloc] initWithJson:[LocalStorage getItemImpl:name]];
}

+(NSString *)getStringItem:(NSString *)name{
    return [[LocalStorage getItemImpl:name] asString];
}

+(Json *)getItemImpl:(NSString*)key{
...
+(void)remove:(NSString*)key;{
    ...
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2([LocalStorage database], [quary UTF8String], -1, &stmt, nil) == SQLITE_OK) {
